...
void SM4Pad(BYTE *Plain, int PlainLength, BYTE* in, int &inLength)
{
    ...
    //padding
    BYTE *temp = (BYTE *)malloc( sizeof(BYTE) * inLength);
    memcpy(temp,Plain,PlainLength);
    for(int i = 0 ; i < 15 - Remainder;i++)
    {
        temp[PlainLength + i] = 0x00;
    }
...
{
    BYTE *PlainAndPad = (BYTE *) malloc(sizeof(BYTE)*((PlainLength / 16 + 1) * 16));
    ...
    CipherLength = len;
	free(PlainAndPad); PlainAndPad=NULL;
}
...
void SM4Decrypt(BYTE *Cipher, int CipherLength, BYTE *Plain ,int &PlainLength, BYTE *Key)
{
    unsigned int subkey[SMS4_ROUND];
    Sms4ExtendKey(subkey,Key);
    BYTE *PlainAndPad=(BYTE *)malloc(sizeof(BYTE)*CipherLength);
    for(int i = 0;i < CipherLength / 16;i++)
    {
